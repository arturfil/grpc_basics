...
.DEFAULT_GOAL := help
.PHONY: sum greet blog calculator help
project := sum greet calculator blog

all: $(project) # Generate Pbs and build

sum: $@
greet: $@ ## generate pbs for greet project
calculator: $@ ## generate pbs for calculator project
blog: $@ ## generate pbs for blog project
...
clean: clean_sum clean_greet clean_calculator clean_blog ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
